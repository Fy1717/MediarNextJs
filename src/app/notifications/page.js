...
const NotificationsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get('http://127.0.0.1:3000/auth/notifications/', {
          headers: {
    ...
    const handleNewNotification = (data) => {
      ...
      setState(prevState => {
        ...
        const isAlreadyListed = prevState.notifications.some(notification => 
  ...
  return (
    <div className="container my-4">
      <NotificationList notifications={state.notifications} onSelectNotification={handleNotificationClick} />
      <NotificationModal
        notification={state.selectedNotification}
        show={state.isModalOpen}
        onHide={() => setState(prevState => ({ ...prevState, isModalOpen: false }))}
      />
    </div>
  );
};
