...
const Article = ({ article: initialArticle }) => {
    ...
    const currentUserID = typeof window !== 'undefined' ? JSON.parse(window.localStorage.getItem('user') || "{}").id : null;
    const [isLikedByCurrentUser, setIsLikedByCurrentUser] = useState(currentUserID && (article.likedUsers.some(user => user.id === currentUserID)));

    useEffect(() => {
        console.log("XXXXXX");
        setIsLikedByCurrentUser(currentUserID && (article.likedUsers.some(user => user.id === currentUserID)));
    ...
    const handleLikeClick = async () => {
        try {
            ...
            const reqUrl = isLikedByCurrentUser ? "http://127.0.0.1:3000/articles/unlikeArticle/" + JSON.stringify(article.id) :
                "http://127.0.0.1:3000/articles/likeArticle/" + JSON.stringify(article.id);
    ...
    const handleRemoveClick = async () => {
        try {
            ...
            const reqUrl = "http://127.0.0.1:3000/articles/deleteArticle/" + JSON.stringify(article.id);
        ...
        <div className="card-body">
...

            {
                window.location.href.indexOf("profile") == -1 ? 
                    <button
                        className={`btn ${isLikedByCurrentUser ? 'btn-warning' : 'btn-primary'}`}
